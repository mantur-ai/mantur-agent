...
已打包应用会在 macOS 的原生应用菜单和 Windows 的帮助菜单中显示当前版本与**检查更新…**。菜单会显示检查中、下载进度、可安装、已是最新版与失败状态；手动检查还会打开本地化的结果或错误对话框。应用启动后会开始检查，并每六小时重复。stable 构建只接收 stable release，版本号含 `alpha`、`beta` 或 `rc` 的构建可以接收预发布版本。只有用户确认后才会下载新版本，下载完成后还需第二次确认，才会准备重启。Host 尚未提供可验证的最终任务日志保存回执，因此自动安装保持阻止状态；此路径不会停止任务。选择稍后会在菜单中保留重启安装操作。在任一对话框等待期间关闭 updater，会取消后续下载或安装。
