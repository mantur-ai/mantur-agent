...
`stopForShutdown()` 冻结提供方注册和委派，取消尚未完成的启动，并等待原始提供方回收、续跑子代理清理及异步生命周期监听器结束。浏览器消息入口在附件存储开始前关闭，停机会等待此前已接纳的保存完成，再拒绝投递。运行结果完成不等于资源释放；成功的 dispose 才释放所有权。清理和监听器失败在运行离开注册表后仍保留。停机期间不投递完成通知或提供方移除通知，续跑子代理取消时保留待处理收件箱输入。Host 必须先启动 agent-loop 停机，再等待本服务及其他资源所有者结束，最后重新验证 writer 封存。提供方启动异常在停机前发生时也会保留。启动若以确切停机取消原因以外的值拒绝，会阻止停机成功；服务不会根据错误消息推断回滚成功。单独的服务结果不能授权安装。
...
- **发布即边界**——发布前提供方拥有设置与回滚；发布后调用方须 dispose（资源释放）运行。服务保留同一回收操作直到其结束，包括提供方移除后。
- **注册受 effect 作用域约束**——普通提供方移除会阻止新启动，不撤销已接受的运行。显式停机通过原始 dispose 关闭仍由服务跟踪的运行。
