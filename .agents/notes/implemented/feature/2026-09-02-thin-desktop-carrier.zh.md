...
DMG 镜像文件保留在构建输出目录，但 `hdiutil` 挂载点使用 macOS `getconf DARWIN_USER_TEMP_DIR` 下的独立临时目录。关闭所有权的外置构建卷可能允许创建镜像，却拒绝在其输出目录中挂载镜像。解析或创建系统挂载点失败会明确报错；打包不会修改卷权限，也不会尝试其他目录。清理只在卸载后移除空挂载点；无法安全移除挂载点时保留镜像文件。
