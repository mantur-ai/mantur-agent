...
`stopForShutdown()` permanently freezes new opens, immediately closes existing domains to new writes, and joins admitted opens and all unit cleanup. Allocation rollback failures remain recorded and reject shutdown. Plugin disposal uses this operation; `closeAll()` closes the current domains while allowing later reopening.
