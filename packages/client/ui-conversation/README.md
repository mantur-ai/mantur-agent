...
A layout can declare `conversation.composer.layout.permissions` with a boolean `disabled` owner prop. Conversation contributes its internal permission selector for that declaration's lifetime, using the same input faces, permission projection, command callback, and interaction locks as the inline control. Framework-bound slot occupancy removes the inline selector only while the alternate slot has a contribution; removing or recreating the declaration restores or relocates the control without changing the Session permission value. The layout's display lock includes the composer block; model-selection lock rules are unchanged.
