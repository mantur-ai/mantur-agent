...
The [account UI](../../../../packages/client/ui-mantur-account/README.md) selects its form from the Host's identity mode, not the presence of a browser global. Native forms and Settings share a revision-checked preload observer. Password and registration inputs are transient; Main owns attempt recovery, fixed-URL reopening, persisted Skip and exact expiry. An absent bridge fails explicitly. Pending registration never reports authentication, and offline revocation remains visible. Real Loader/browser tests use a simulated public preload and do not establish Electron or real-site acceptance. Marketplace login routing remains incomplete.
