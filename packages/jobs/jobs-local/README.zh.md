...
`stopForShutdown()` 冻结新任务、抑制未报告的完成通知，并等待生产者释放资源及异步完成监听器结束。停止期间，所有者和服务释放也会先等待对应完成监听器，再释放记录。生产者、监听器和清理取消的失败在记录离开注册表后仍保留。该方法不取消工作；本地执行所有者必须另行停止资源，之后的作用域释放也不会取消远程任务。安装前，Host 必须持久保留远程任务标识，并证明其他生产者均已停止。
