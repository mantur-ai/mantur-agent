...
生产集成复用剧本工作台的单一外框和现有主会话。外框声明明确的资产标签/内容子槽，同会话切换时保留已激活的资产面板。仅供测试的资产贡献通过 Cordis Loader 加载，从这些子槽驱动隔离发送器；不注册第二个外框或发布资产提供者。集成需要项目清单适配器、源数据原子更新、关联结果事件及现有 Base 读回。受控 Worker 仅验证隔离交互协议，不代表模型或生产线。预览与草稿编辑独立于登录。
