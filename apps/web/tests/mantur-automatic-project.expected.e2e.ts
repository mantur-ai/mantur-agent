...
it.skipIf(webSnapshotMode() === 'record' || process.platform === 'win32')('resumes a lost project response after reload and sends the restored Skill and image once', async () => {
  ...
  try {
    ...
    // Substitute the preload capability for browser coverage; no Electron window or OS chooser runs here.
    const desktopPicker = vi.fn<() => Promise<string | null>>().mockResolvedValue(null)
    await page.exposeFunction('testDirectoryPick', () => desktopPicker())
    await page.addInitScript(() => {
      const host = window as unknown as { testDirectoryPick: () => Promise<string | null> }
      Object.assign(window, { manturDirectoryPicker: { pick: () => host.testDirectoryPick() } })
    })
    ...
    await pathRow.getByText(projectRoot, { exact: true }).waitFor()
    await pathRow.getByRole('button', { name: '更改', exact: true }).click()
    await expect.poll(() => desktopPicker.mock.calls.length).toBe(1)
    await pathRow.getByRole('button', { name: '更改', exact: true }).waitFor()
    expect(saveRoot).toHaveBeenCalledTimes(rootSaveCalls)
    expect(scaffold.ctx.manturProjects.settings()).toEqual({ source: 'custom', rootPath: projectRoot })
    desktopPicker.mockRejectedValueOnce(new Error('Desktop picker unavailable'))
    await pathRow.getByRole('button', { name: '更改', exact: true }).click()
    await pathRow.getByRole('alert').waitFor()
    expect(desktopPicker).toHaveBeenCalledTimes(2)
    expect(saveRoot).toHaveBeenCalledTimes(rootSaveCalls)
    expect(scaffold.ctx.manturProjects.settings()).toEqual({ source: 'custom', rootPath: projectRoot })
    await pathRow.getByRole('button', { name: '重试', exact: true }).click()
    await pathRow.getByText(projectRoot, { exact: true }).waitFor()
    expect(desktopPicker).toHaveBeenCalledTimes(2)
    const desktopRoot = join(scaffold.workspaceCwd, 'desktop-projects')
    desktopPicker.mockResolvedValueOnce(desktopRoot)
    await pathRow.getByRole('button', { name: '更改', exact: true }).click()
    await pathRow.getByText(desktopRoot, { exact: true }).waitFor()
    expect(desktopPicker).toHaveBeenCalledTimes(3)
    expect(picker).toHaveBeenCalledTimes(hostPickerCalls)
    expect(saveRoot).toHaveBeenCalledTimes(rootSaveCalls + 1)
    expect(saveRoot).toHaveBeenLastCalledWith(desktopRoot)
    expect(scaffold.ctx.manturProjects.settings()).toEqual({ source: 'custom', rootPath: desktopRoot })
    const persistedRoot: unknown = JSON.parse(await readFile(join(scaffold.workspaceCwd, '.dsh-storages', 'mantur_projects.json'), 'utf8'))
