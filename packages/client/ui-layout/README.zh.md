...
可选的 `main.workbench` 插槽在对话旁打开，也支持尚无 Session 的首页。`openWorkbench()` 清除已选主页面；`closeWorkbench()` 释放工作台内容且不重新挂载对话。工作台打开时，可从对话区域下角调整宽度。工作台按所选 Session 和未选择 Session 的首页分别记住显示状态，刷新后重置。切换会话会卸载原编辑器，返回时恢复该会话的打开状态。
