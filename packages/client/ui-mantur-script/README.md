...
The plugin is the sole registrant of `main.workbench` and `main.workbench.toggle`. Its declared editing child slots accept the optional [editing plugin](../ui-mantur-editing/README.md). The editing tab exists only while that plugin is mounted. Selecting content does not alter panel visibility. After first activation, both content panels remain mounted across same-Session tab switches and collapse. The layout releases the panel instance on a Session switch; the registration store retains document drafts by Session and file for later remounts. Removing editing leaves the script shell intact; an unavailable selected editing panel reports that state without selecting another panel.
