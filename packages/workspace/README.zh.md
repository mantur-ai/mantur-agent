...
workspace 组提供宿主 UI 背后的持久项目列表。`workspace` 注册用户目录并归集会话；`mantur-projects` 使用持久重试标识在首次发送时准备目录。UI 可以移除项目注册而不删除文件夹或会话历史。这些 Host 包不注册工具、提示词或会话事件。
...
| [`mantur-projects`](mantur-projects/README.zh.md) | 准备首次发送的项目目录，并在重试间保留其标识 | `ctx.manturProjects` |
