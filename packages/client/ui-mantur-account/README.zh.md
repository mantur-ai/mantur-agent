...
原生 preload 发布带版本号的公开账号元数据，绝不发布设备 bearer。密码和注册验证码只作为临时表单输入及具名 IPC 参数存在，不进入可观察 store。原生能力缺失或身份模式查询失败时明确报错，不切换到独立凭据。同一个客户端订阅服务引导、设置页和按需账号弹窗，并每两秒轮询等待中的浏览器授权。已被取代的操作或低于已接收 Main 版本号的回包不能修改账号状态、错误或表单操作结果；相同版本号的回包仍报告当前操作的结果。重新打开使用已保存的 attempt；密码、注册和验证码请求绝不自动重试。

按需原生弹窗使用 `shell.overlay` 和同一账号表单。“返回创作”和 Escape 关闭界面，不取消 Main 已接受的操作。明确执行且成功的“暂时跳过”关闭弹窗；之前保存的跳过选择不会关闭新打开的界面。重复打开共享同一个结果。其他活跃弹窗、进行中的账号操作或所有者缺失都会明确失败。卸载会拒绝未完成请求并释放观察者；迟到结果不会重新打开已关闭的弹窗。

原生登录、注册和账号设置的已有插图位使用透明招手吉祥物，尺寸为 72×72，紧凑窗口为 48×48；蓝色无限 Logo 仍是产品标识。
