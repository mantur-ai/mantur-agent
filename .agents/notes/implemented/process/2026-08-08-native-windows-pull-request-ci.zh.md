...
每条原生 Vitest 命令都占用独立的阻断性 `pwsh` 步骤。[Actions shell 包装层](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#exit-codes-and-error-action-preference)报告最后一条原生命令的退出码，因此合并调用会让后续成功掩盖先前的 worker 失败。工作流测试固定这两个单命令步骤，并禁止覆盖失败结果；先前失败会让后续步骤跳过，而不会被其退出码替换。测试选择和预算保持不变。这恢复的是失败报告，不是 worker 稳定性。
