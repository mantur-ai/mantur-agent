...
The Mantur navigation plugin occupies generic conversation slots for creation choices below the headline, contextual guidance above the resident composer, and composition of owner-created heading, workspace, and content nodes. Host settings persist the last mode and explicit dismissal. Mode selection changes only the catalog recommendation filter and locale-owned guide copy; it never selects an Agent preset or adds model context.

The visible modes are Script writing, Drama production and Asset production. Settings validation maps the known retired `editing` value to `production`; unknown values remain invalid. This resolution does not emit a mode-selection event, open the workbench or change the draft, project or Session.
...
Mantur declares the optional-Session `conversation.composer.bar.accessory.permissions` child in the input card's accessory slot. Conversation supplies its internal selector using the original input state, permission projection, command callback, and shared interaction locks. The composer passes its display lock, including the composer block. Slot occupancy selects the accessory or toolbar location without duplicating permission state. Actual DOM placement before the editor keeps focus order aligned and avoids visual-only relocation from the footer.

The product composition supplies ordered real marketplace slugs. The browser intersects those lists with the current Host catalog and keeps fewer recommendations when matching Skills are absent. Catalog failures remain visible with Retry. An installed shortcut uses the input facade to append a titled reference, deduplicate its source/ref identity, and focus the editor without submitting. An uninstalled Skill first shows a brief confirmation, not its description or introduction. Installation must explicitly succeed before insertion. Closing or replacing the confirmation, switching Sessions, leaving for a marketplace page, or unmounting invalidates the pending insertion. A plugin-local navigation generation advances before page navigation, so a late installation cannot insert before a render or revive on return. More skills uses the same actions. Marketplace Use retains the separate new-Session behavior described in the [marketplace decision](2026-09-03-mantur-marketplace-navigation.md).

Recommendation buttons and their inserted references use the same curated localized short name, at most four Chinese characters. Tooltips retain catalog titles; missing recommendation mappings produce an explicit configuration warning. More skills preserves full list titles, and entries without a curated mapping retain that title when inserted. Short names never change Skill IDs or model-visible references.

The recommendation rail uses 40px-high bordered buttons near the composer. It retains native horizontal scrolling and keyboard focus without a visible scrollbar. Local DOM measurements show edge arrows and fades only while the contents exceed the available width; the controls scroll a visible rail width and disable at each end. These measurements hold no catalog, mode, or input state.

馒头仔 presents fixed copy in a fixed-position bubble anchored above/right of the mascot. It does not occupy a layout row, so its visibility and copy cannot move the recommendation row or composer. A constrained gap below the mode tabs uses the space beside them when it provides at least 180px of reading width; narrower windows keep the scrollable body below the tabs. This avoids clipping guidance to less than one line at the desktop's 880×600 minimum without changing text size, composer layout, or distance above the mascot. The close button stays outside the scroll area. Dismissal persists without forcing the bubble open after mode changes. The mascot and its guide render only on the conversation home and unmount during active chat. The [Mantur brand identity](2026-09-02-mantur-brand-identity.md) remains unchanged; the assistant is not a replacement product logo.
...
Every Web build includes the approved 2x/3x transparent mascot assets because the shipped Mantur overlay can load its UI plugin against the common frontend. The existing mode setting selects the three peeking illustrations without a second mode state. A fixed 184×120 canvas aligns its y=104 contact line with the composer border; the square welcome illustration serves existing native account illustrations without squeezing a peeking pose into a square. Original artwork remains archived, with no runtime substitution for missing assets. Component tests pin the mapping; built-browser checks cover asset decoding, contact geometry, mode persistence and draft preservation. Document metadata, favicon, and install-manifest branding remain profile-owned.
