...
it('returns from marketplace authorization with the selected detail and complete unsent draft intact', async () => {
  ...
  try {
    ...
    await page.getByRole('img', { name: 'native-reference.png', includeHidden: true }).waitFor()
    ...
    captures.push(`## Native account from marketplace\n\n${await account.ariaSnapshot()}`)
    ...
    for (const kind of ['local', 'network', 'endpoint-unavailable', 'protocol'] as const) {
      ...
      expect(await page.getByRole('img', { name: 'native-reference.png', includeHidden: true }).count()).toBe(1)
    }
    await detail.getByRole('button', { name: '登录后安装' }).click()
    await account.getByRole('button', { name: '登录漫途账号', exact: true }).click()
    ...
    captures.push(`## Marketplace after Skip\n\n${await detail.ariaSnapshot()}`)
    await detail.getByRole('button', { name: '登录后安装' }).click()
    await account.getByRole('button', { name: '返回创作' }).click()
    await detail.getByRole('button', { name: '登录后安装' }).waitFor()
    await expectDialogFocus(detail, '登录后安装')
