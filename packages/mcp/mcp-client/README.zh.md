...
服务器连接断开时——例如本地服务器进程崩溃——插件会以从 500 ms 起逐次翻倍、上限 30 s 的延迟自动重连，并刷新工具集；重连进度在日志中可见。stdio 中断期间最后已知的工具仍会列出，但对它们的调用会失败，直到服务器恢复。连续失败十次后，该服务器的工具会被移除，重连停止，直到你重载配置或重启 harness；服务器持续连接一段时间后，该计数会重置。设置 `reconnect.enabled: false` 可禁用自动重连——此时 stdio 工具在断开后仍会列出，但调用失败，直到你重载。编辑配置项会在原地重载服务器连接，未变的名称保持不变。

SDK 对当前已初始化的 Streamable HTTP 会话报告 HTTP 404 时，监督器会停用该连接并移除其工具。监督器等待本地关闭完成后，按既有重连策略初始化新会话并发现工具，不会重试失败的工具调用。新会话不继承服务端项目绑定或草稿归属；明确继续修改前应核对已保存状态。启动端点失败、401/403、JSON-RPC 错误和工具错误不表示会话已失效。禁用重连时，失效 HTTP 工具保持移除，直到重新加载。
...
`apply` 解析重连策略、在当前注册作用域内预留 `serverName`、启动监督器，并等待初始连接加发现完成。独立 Agent 作用域可以复用相同 namespace，因为其工具与传输彼此隔离；同一作用域内重复会在加载时失败。监督器把所有同步——初始、通知与重连——串行到同一条队列，因此两次同步绝不会交错执行各自的先 dispose 后注册交换。[自动重连 Agent Note](../../../.agents/notes/implemented/feature/2026-08-06-mcp-client-auto-reconnect.zh.md) 拥有重连决策。
...
<a id="owned-shutdown"></a>
### 所属连接的关闭

`connectMcpServer(ctx, config, beforeClose?)` 挂载同一个作用域连接，并返回就绪及关闭句柄。`stopAccepting()` 同步拒绝新的执行并停止重连，随后等待已接收调用及其结果图片写入，不中止调用信号，也不使其连接世代失效。调用方必须在等待完成之前保留 Agent、模型能力与附件服务。

`dispose()` 等待已接收调用、可选的远端所有者 `beforeClose`，然后关闭传输，等待连接和同步工作完成后注销工具。作用域释放只在清理成功后释放名称预留。每个关闭阶段使用 `toolCallTimeoutMs`；传输关闭保留既有五秒上限，同时要求关闭 Promise 和关闭通知完成。重复调用保留首次结果。执行、取消、附件存储、所有者排空和传输清理失败都会拒绝关闭；排空超时不会取消远端工作，也不允许安装。能力与无效图片拒绝仍是普通文本结果。

HTTP 传输关闭不能证明远端编辑器已经保存工程或完成付费任务。应用通过既有连接提供这一前置条件；MCP 桥接不增加替代协议，也不重放工具调用。[所属连接关闭决策](../../../.agents/notes/implemented/bug-fix/2026-09-07-mcp-owned-shutdown.zh.md) 记录失败与归属保证。
...
初始发现成功后，每个已声明的 MCP 工具都会显示为名为 `mcp__<serverName>__<rawName>`（或其确定性规范化形式）的原生工具，并携带服务器提供的描述与输入 schema。成功的重新同步——包括自动重连后的同步——会替换整个世代；HTTP 会话失效、对插件执行 dispose（资源释放）或重连预算耗尽会移除该世代。
...
已发现工具集合及其 schema 不变时，工具定义前缀保持稳定。增加、移除、重命名或更改工具的重新同步会替换定义，并可能使从第一个变化的 schema token 起的复用失效；恢复未变列表的重连会生成完全相同的定义；HTTP 会话失效会在恢复期间移除列表，因此会使前缀改变两次。
...
- **启动与发现超时继承自 MCP SDK**——插件不暴露连接或发现超时；每次 `initialize` 与分页 `tools/list` 请求都使用 SDK 默认的 60 秒请求超时，因此无响应的服务器或 cursor chain 可能延迟激活。关闭等待超过自身期限会拒绝，而不是宣称同步已完成。
- **HTTP 恢复只识别会话失效**——已建立会话的 SDK HTTP 404 在本地关闭后触发重新初始化；其他 HTTP 请求错误不会触发监督器。本地关闭不能证明结果不明的远端修改已经回滚。
...
- 由 DSH 显式拥有的连接与发现超时是开放的探索方向；SDK 的 60 秒默认值约束着每次启动请求。
