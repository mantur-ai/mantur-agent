...
馒头仔 uses fixed localized copy and the approved transparent blue-scarf/clapperboard artwork shipped in the Mantur Web assets. Its bubble occupies space above the shortcut row, closes with its close button or Escape, and does not reopen when a closed guide changes mode. The first message collapses the welcome region while keeping a compact helper entry. An installation completed after switching Sessions does not insert into either conversation.
