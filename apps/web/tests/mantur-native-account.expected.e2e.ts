...
it('returns from both native marketplace entrypoints with the selected detail and complete unsent draft intact', async () => {
  ...
  try {
    ...
    const guide = page.getByRole('dialog', { name: '短剧编剧', exact: true })
    const detail = page.getByRole('dialog', { name: skill.name, exact: true })
    const account = page.getByRole('dialog', { name: '登录漫途账号', exact: true })
    const expectDialogFocus = (dialog: ReturnType<Page['getByRole']>, name: string) => expect.poll(() => dialog.getByRole('button', { name, exact: true })
      .evaluate(element => document.activeElement === element)).toBe(true)
    const captures: string[] = []
    await page.getByRole('button', { name: '短剧编剧', exact: true }).click()
    const guideInstall = guide.getByRole('button', { name: '登录后安装' })
    await guideInstall.evaluate((element) => { element.focus() })
    ...
    await guideInstall.waitFor()
    await expectDialogFocus(guide, '登录后安装')
    await guideInstall.click()
    await account.getByRole('button', { name: '暂时跳过' }).click()
    await guideInstall.waitFor()
    await expectDialogFocus(guide, '登录后安装')
    captures.push(`## Guide after Skip\n\n${await guide.ariaSnapshot()}`)
    await guide.getByRole('button', { name: '关闭引导' }).click()
    ...
    await expectDialogFocus(detail, '登录后安装')
    await detail.getByRole('button', { name: '登录后安装' }).click()
    ...
    await expectDialogFocus(detail, '安装技能')
    ...
    await page.getByRole('button', { name: '短剧编剧', exact: true }).click()
    await guide.getByRole('button', { name: '安装后使用' }).waitFor()
