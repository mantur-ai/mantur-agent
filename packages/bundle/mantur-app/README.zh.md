...
协调器为每次回执检查整个根的 worker 和动态激活历史，即使所有提供方实例已经消失。缺少历史仍会拒绝。在 Shell 完成策略未明确时，保留既有 codeRuntime 存在门禁和 Host runner 模块排除规则；执行服务从未使用也不会启用安装。默认 profile 安装及真实剪辑/MCP 组合仍未验证；Host 阶段顺序已有受控回调和持久化测试。
