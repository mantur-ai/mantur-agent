...
The optional `main.workbench` slot opens beside the conversation, including on the home screen before a Session exists. `openWorkbench()` clears the selected main page; `closeWorkbench()` hides the opened occupant without unmounting it or the conversation. The current Session retains at most one opened occupant; a never-opened workbench is not mounted. The conversation width can be resized from its lower corner while the workbench is open. Workbench visibility is remembered separately for each selected Session and the unselected home screen, and resets on reload. Switching away unmounts its editor; returning restores that conversation’s open state.

The optional `main.workbench.toggle` slot sits at the conversation's right edge, outside the hidden workbench subtree. It receives `expanded`, `openWorkbench`, and `closeWorkbench`; its occupant owns the control and localized labels.
...
One `register()` call contributes `AppFrame` into the runtime's built-in `'root'` slot and declares `sidebar`, `conversation`, `details`, `main.page`, `main.workbench`, `main.workbench.toggle`, and `shell.overlay`, seats the layout store, and wires the `ctx.layout` panel-action service. The transient layout store starts the sidebar at its default width, details closed, and no main page selected, and never reads or writes `localStorage`. AppFrame always keeps the conversation and details occupants mounted; selecting a branded `MainPageId` hides the conversation, renders the `main.page` occupant, and temporarily renders details at zero width until the page closes. It projects the selected Session title over the build-configured product title or the localized `common.brand.localBuild` fallback, so locale revisions update document metadata with the root entry. The theme presenter is a second effect: pure DOM writes from resolved snapshots — initial state through the getter once, then event-driven only, with no React path. It applies palette, font-size, and token variables before measuring the rendered background as the single color authority.
