...
The home workspace picker sits in a left-aligned footer directly below the input card, followed by the permission selector with wrapping on narrow screens. Conversation supplies the permission control through the layout's declared child slot; Mantur owns no permission state. It uses the conversation owner's existing workspace control and switch action, preserving draft and attachment transfer. Keyboard focus reaches the footer after the input controls; the official composition keeps its original order.
...
馒头仔 uses fixed localized copy and approved transparent artwork shipped in the Web assets. Each creation mode selects its matching 184×120 peeking illustration, with the artwork's y=104 contact line aligned to the input card. The compact conversation helper uses the square welcome illustration. Its fixed-position bubble sits above/right of the mascot without adding a layout row; opening, closing, or changing mode keeps the shortcut row and input card stationary. When the gap below the mode tabs clips the body, the bubble moves beside the tabs if at least 180px of width remains. Otherwise it scrolls within that gap. Both placements keep the original text size, visible close button, and 8px separation above the mascot. The close button and Escape persist dismissal; a mode change does not reopen a closed guide. The first message collapses the welcome region while keeping a compact helper entry. An installation completed after switching Sessions does not insert into either conversation.
