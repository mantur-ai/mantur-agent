...
可选的 `main.workbench` 插槽在对话旁打开，也支持尚无 Session 的首页。`openWorkbench()` 清除已选主页面；`closeWorkbench()` 隐藏已打开的工作台，不卸载工作台或对话。当前 Session 最多保留一个已打开的工作台；从未打开的工作台不会挂载。工作台打开时，可从对话区域下角调整宽度。工作台按所选 Session 和未选择 Session 的首页分别记住显示状态，刷新后重置。切换会话会卸载原编辑器，返回时恢复该会话的打开状态。

可选的 `main.workbench.toggle` 插槽位于对话右侧边缘，在被隐藏的工作台子树之外。它接收 `expanded`、`openWorkbench` 和 `closeWorkbench`，占位组件负责控件及其本地化标签。
