...
`stopForShutdown()` 停止接收新的组合、切换、继承和编写调用，并等待已接收的操作结束。现有常驻插件树继续保留：Host 在排空后枚举 `livePresetMounts(ctx.fiber)`，分别停止其中的所有者，包括已被替代的旧代次。此方法等待操作结束，但不替换调用方收到的结果；它不关闭插件所有者，也不生成安装回执。
