...
The native sidebar footer displays a yellow bun icon and Mantou number without a card or refresh button. While visible, it reads at the Host-configured `balanceRefreshIntervalMs` cadence (5 seconds in the Mantur profile), and immediately after sign-in, account changes, window focus or network reconnection. Pending refreshes retain the confirmed number with an accessible busy state; failures clear it. Missing balances display “—” with localized status in the tooltip. Hidden pages pause polling; returning refreshes immediately. Logout and account replacement invalidate pending reads.
