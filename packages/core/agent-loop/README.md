...
`quiesceForShutdown()` freezes admission and joins driver and startup work while published session writers remain open for admitted Host operations. The Host drains those operations before calling `stopForShutdown()`. `stopForShutdown()` permanently freezes admission and joins tracked startup operations, including work that outlives public creation cancellation. It preserves unexecuted inbox input and returns final exclusive writer offsets only after closure; earlier writer failures and writes attempted after sealing invalidate the result. `verifyShutdown()` requires an existing shutdown and rechecks sealed writers after its completion, rejecting any later append attempt. The Host must call it after joining all other producers. These offsets cover agent-loop writers, not all Host producers: Cordis scope disposal alone does not prove successful resource cleanup, and the Host must obtain separate producer results before issuing an installation receipt.
