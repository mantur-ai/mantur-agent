...
shell 和终端消费方在分配进程前通过 command-scopes 准备身份。完整进程树清理与 Main 释放确认先于命令完成；取消中的准备也会等待迟到分配完成清理。真实 Main、Loader、provider、Bash 和固定解包 CLI 的测试覆盖余额、未登录隔离和流式退出登录取消。真实持久终端路径跨多次发送保留描述文件，并等待退出登录清理。终端测试仅替代空闲 agent 所有者，不替代 Loader、后端或操作系统进程。原生表单、打包 CLI 调用与原生操作系统验收仍未完成。
