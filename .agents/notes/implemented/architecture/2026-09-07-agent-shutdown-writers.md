...
Settings shutdown belongs in `packages/settings/settings/src/index.ts` and `packages/settings/settings-file/src/index.ts`: namespace writes and observer callbacks can outlive the file operation queue. A ready provider stops both producers and joins both queue layers plus watcher close. Started callbacks, held document creation, repeated stop, and watcher-close failure verify upgrades. Startup composition must settle before a ready-provider shutdown is requested.

Goal-round driver shutdown belongs in `packages/goal/goal-round-driver/src/index.ts`: its scheduling promises and reservations are private to the producer. The explicit lifecycle handle freezes automatic triggers and retains original driver tasks independently of live-agent lookup. Its step fence remains until plugin disposal; admitted rounds cancel with pending input preserved. The Host freezes agent admission before stopping this producer. After agent admission freezes, AgentLoop retains the original inbox claim and restores it; the goal driver does not mutate the frozen inbox. Held checkpoints, later manual work, and interrupted pre-step hooks during joint AgentLoop shutdown with a real JSONL writer verify upgrades.

Preset admission shutdown belongs in `packages/preset/agent-presets/src/index.ts`: resolving a cold transcript can allocate a standing plugin tree after the Host first reads its owners. The explicit stop freezes mutating and mounting calls, joins admitted operations, and retains installed generations for a subsequent `livePresetMounts(rootFiber)` inventory. Caller-visible operation failures remain with those callers; this drain alone does not certify plugin cleanup. Held cold-read allocation and post-stop composition and authoring rejection verify upgrades.

Profile patch-watcher shutdown belongs in `packages/boot/app-boot/src/index.ts`: watcher opening can finish after admission freezes, and refresh callbacks can still recompose the root include. Root-scoped ownership joins opening before closing every registered watcher and retains close failures across repeated stop calls. Held opening and cleanup, opening rejection, and synchronous close failure verify upgrades. Module HMR and application disposal remain separate owners.

Native identity cleanup belongs in `packages/credentials/authorization-manturhub/src/index.ts`: the private connection owns brokered response bodies and command leases. Its connection effect and explicit `stopNativeForShutdown()` share one operation that withdraws command registration, drains its consumers, and closes the connection after its receipts settle. Failed command cleanup does not bypass connection cleanup, and repeated callers receive the retained failure. Standalone mode rejects this native-only operation. Held command receipts after real process exit and held response cancellation verify upgrades; Main retains IPC until those receipts complete.
