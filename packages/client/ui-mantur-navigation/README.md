...
- Native desktop update checks remain above Settings in idle, current, active, and failed states, including the collapsed rail. The entry shows the installed version; only a successful check reports that it is current. Browser builds expose no installer. Downloads require an explicit click; restart confirmation and save failures remain owned by the native controller. Installation is blocked pending a verified final Host checkpoint.
