...
`stopForShutdown()` 拒绝新的定义、激活、Client 源码请求和 handler 调用，取消待审批请求，并在正常回收插件前等待已接纳的激活与 handler 调用。重复关闭仍保留回收失败。`hasStartedPrograms` 与 `hasStartedDynamicPrograms(ctx)` 在整个 Host 根内保留激活历史，不因 stop、undefine 或提供方替换而清除；仅定义或等待审批不记录为执行。受管理的关闭不证明任意后台工作或 OS 后代已经停止。
