...
it('loads asset and Clip operator outputs and previews real image and video bytes through RPC', async () => {
  ...
  try {
    ...
    await mkdir(join(workspace, '剧本'))
    const scriptPath = join(workspace, '剧本/第一集.md')
    await writeFile(scriptPath, '# 第一集\n\n林夏：你终于来了。\n')
    ...
    expect(await page.getByLabel('项目内文件夹').count()).toBe(0)
    await page.getByRole('button', { name: '编辑', exact: true }).click()
    await page.getByRole('textbox', { name: '编辑', exact: true }).fill('# 第一集\n\n林夏：我等你很久了。\n')
    await page.getByRole('button', { name: '保存', exact: true }).click()
    await expect.poll(() => readFile(scriptPath, 'utf8')).toContain('我等你很久了')
    await page.getByRole('button', { name: '阅读', exact: true }).click()
    const scriptAria = await captureStableAria(page, '[aria-label="剧本工作台"]', scaffold.workspaceCwd)
    await compareOrRefreshGolden(fileURLToPath(new URL('./expected/mantur-script.md', import.meta.url)), scriptAria, webSnapshotMode())
