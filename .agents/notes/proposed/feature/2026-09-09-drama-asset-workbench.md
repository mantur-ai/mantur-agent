...
Production integration reuses the script workbench's single shell and existing main session. The shell declares explicit asset tab/content child slots and retains an activated asset panel across same-Session switches. A test-only asset contribution loads through Cordis Loader and drives the isolated sender from those slots; no second shell or shipped asset provider is registered. Integration requires a project manifest adapter, atomic source updates, correlated result events and existing Base readback. A controlled Worker proves the isolated interaction protocol only; it is not a model or production pipeline. Preview and draft editing remain independent of login.
