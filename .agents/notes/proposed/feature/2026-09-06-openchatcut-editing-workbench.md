...
The distributable desktop proposal builds only on the matching native runner and pins the OpenChatCut commit, upstream tree, three patch digests and result trees, npm package integrities, Chrome archive, FFmpeg binary, whisper.cpp v1.9.2 commit, and the official CMake macOS archive. CMake remains in the build cache and is not installed globally. The generated read-only `resources/mantur-cut` tree contains the embedded server, editor assets, Remotion bundle and compositor, Chrome Headless Shell, FFmpeg, ffprobe, Whisper CLI and server, production dependencies, source identity, patches, retained licenses, dependency inventory, build and tool versions, content hashes, and production audit. Production dependencies live below `runtime/node_modules` because electron-builder excludes a root extra-resource directory named `node_modules`; package bin links are build-only and are removed before hashing. Electron supplies the Node 24 runtime through `ELECTRON_RUN_AS_NODE=1`; the package does not carry another Node installation. Runtime code must resolve every declared relative path inside this resource root and must not download or use a development fallback. macOS packaging uses electron-builder for signing and the update ZIP, then creates the DMG and blockmap with `hdiutil` under a temporary unique volume name before restoring the product volume name. Application data remains under the installed application's user-data and project directories.
