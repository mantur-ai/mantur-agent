...
`stopForShutdown()` rejects new definitions, activations, Client source requests and handler calls, cancels pending approvals, and waits for accepted activations and handler calls before normal plugin retraction. Retraction failures remain failures on repeated shutdown. `hasStartedPrograms` and `hasStartedDynamicPrograms(ctx)` retain activation history across stop, undefine and provider replacement within the Host root; definitions and unanswered approvals alone do not mark execution. Managed shutdown does not prove arbitrary background work or OS descendants have stopped.
