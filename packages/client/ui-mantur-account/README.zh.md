...
原生 preload 发布带版本号的公开账号元数据，绝不发布设备 bearer。密码和注册验证码只作为临时表单输入及具名 IPC 参数存在，不进入可观察 store。原生能力缺失或身份模式查询失败时明确报错，不切换到独立凭据。同一个客户端订阅服务引导和设置页，并每两秒轮询等待中的浏览器授权。已被取代的操作或低于已接收 Main 版本号的回包不能修改账号状态、错误或表单操作结果；相同版本号的回包仍报告当前操作的结果。重新打开使用已保存的 attempt；密码、注册和验证码请求绝不自动重试。
