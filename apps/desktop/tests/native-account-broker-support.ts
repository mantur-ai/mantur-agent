...
export async function nativeBrokerBench(api: (request: IncomingMessage, response: ServerResponse) => void,
  ...
  const account = { id: randomUUID(), email: 'broker@example.com', display_name: 'Isolated account' }
  ...
  const server = createServer((request, response) => {
    ...
    if (request.url === '/api/v1/native/auth/attempts') {
      const chunks: Buffer[] = []
      request.on('data', (chunk: Buffer) => { chunks.push(chunk) })
      request.on('end', () => {
        const input = JSON.parse(Buffer.concat(chunks).toString('utf8')) as { credential_verifier: string; device_instance_id: string }
        verifier = input.credential_verifier
        device = input.device_instance_id
        send({ status: 'pending', attempt_id: id, user_code: 'ABCD-EFGH', verification_uri: `${origin}/auth/agent`,
          verification_uri_complete: `${origin}/auth/agent?user_code=ABCD-EFGH`,
          attempt_expires_at: new Date(now.value + 600_000).toISOString(), expires_in: 600, poll_interval: 2 }, 201)
      })
    } else if (request.url === `/api/v1/native/auth/attempts/${id}/password`) {
      request.resume()
      request.on('end', () => { send({ status: 'ready', credential_id: grantId, expires_at: expiry, account }) })
    } else if (request.url === '/api/v1/native/auth/credentials/activate') {
      const candidate = request.headers.authorization?.slice(7)
      if (candidate === undefined || nativeVerifier(candidate) !== verifier) { send({ error: 'CREDENTIAL_INVALID' }, 401); return }
      bearer = candidate
      request.resume()
      request.on('end', () => { send({ status: 'active', credential_id: grantId, expires_at: expiry }) })
    } else if (request.url === '/api/v1/native/session/revoke') {
      revoked = true
      response.writeHead(204).end()
    } else if (request.url === '/api/v1/native/session') {
      send({ status: 'active', credential_id: grantId, expires_at: expiry, account, device_instance_id: device,
        client_id: 'mantur-agent', issued_at: new Date(now.value).toISOString(), last_used_at: new Date(now.value).toISOString() })
    } else {
  ...
  const controller = new NativeAccountController(store, http, { environment: 'test', deviceName: 'Isolated broker test',
    platform: 'macos', now: () => now.value, openBrowser: async () => { throw new Error('No system-browser operation in this fixture') } })
  ...
  await controller.password({ email: account.email, password, consent: true })
  return { root, controller, broker, store, origin, observed, now, expiry, password, bearer: () => bearer }
}
