...
The third editor patch owns this behavior above the frozen base and packaged patches. The shutdown implementation preserves the audio finalizer and public MCP protocol; dependency pins are owned by the [dependency security decision](2026-09-08-mantur-editor-security-dependencies.md). The integration records exact media children at owned spawn sites; it does not scan machine processes. The owned Remotion wrapper preserves browser-close failures and records actual browser acquisition. Remotion 4.0.509 supplies no supported process-and-pipe close evidence, so acquisition prevents successful shutdown even if its close promise resolves.
...
The runtime regression controls stop acknowledgement, process exit, late stderr and close as separate events. Real subprocess tests still cover startup and shutdown; the ordering test does not require a descendant to inherit a Windows pipe. Native account expiry uses a controlled clock after listener startup, and disconnected-channel fixtures restore the test runner IPC synchronously before returning.
