...
Main owns client-session browser login, refresh and OS-encrypted account storage. Production defaults to `https://hub.mantur.ai`. The [account owner](src/auth/README.md) defines cancellation, logout and reauthorization.

Client-session tokens are not interchangeable with retired device-grant records. Upgrading requires browser authorization; existing conversations, Skills and model credentials are unchanged.

The [embedded CLI input](cli-runtime/README.md) is a reviewed archive plus an independent npm lockfile. Development and packaging prepare `mantur-cli` resources. Main creates a profile-local launcher, prepends it to the supervised Host's PATH, and uses its own Electron executable in Node mode. Missing resources fail startup. Commands use broker-v2 descriptors; only Main attaches the dedicated API Key upstream. Runtime never installs or searches for a global CLI.
...
- The native icon source is a 1024 px RGBA PNG with transparent rounded corners and a centered blue infinity emblem. The Web client uses the transparent logo separately. macOS and Windows packages derive their platform icon formats during the native build; a vector source remains unavailable.
