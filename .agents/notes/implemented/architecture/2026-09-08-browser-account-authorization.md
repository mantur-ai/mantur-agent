...
The desktop retains `漫途Agent` as Electron's storage identity independently of visible branding. Electron derives the macOS Keychain service from `app.getName()`; changing this identity makes existing OS-encrypted grants unreadable. Main sets the storage name before opening account storage. No credential copying or alternate decryption path is needed.
...
Local HTTP, SQLite, callback, Loader and CLI tests exercise controlled data. Electron Node-mode CLI balance verification uses a local fixture response, not a real account or the Java service. Simulated-preload browser snapshots prove presentation and draft preservation only. A macOS development-client check verified reading an existing Keychain-encrypted grant and fetching the real account balance after restoring the storage identity. Same-version Java/PostgreSQL 16 integration, new website consent, Windows DPAPI and ACL execution, packaged resources and platform-specific browser return remain acceptance work. No local result authorizes remote deployment or real Key creation.
