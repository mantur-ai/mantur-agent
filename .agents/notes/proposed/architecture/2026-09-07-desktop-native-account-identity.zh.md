...
[账号界面](../../../../packages/client/ui-mantur-account/README.zh.md)根据 Host 身份模式选择表单，不根据浏览器全局对象是否存在进行猜测。原生表单与设置页共享检查版本号的 preload 观察者。密码和注册输入只临时保留；Main 负责 attempt 恢复、固定地址重开、持久跳过和精确到期。桥接缺失时明确失败。待激活注册不报告已认证，离线撤销持续可见。真实 Loader 与浏览器测试使用模拟公开 preload，不能证明 Electron 或真实站点已验收。广场登录路由仍未完成。
