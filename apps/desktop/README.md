...
Confirmed updates wait for message submission and automatic-project preparation before locking and saving drafts. A failed submission cancels that update and retains the input. The native waiting dialog offers Cancel update; cancellation leaves the running Host untouched, and the renderer receipt deadline still bounds the wait. Switching draft ownership while waiting also cancels the request. The installer cannot proceed on a cancelled, failed, or stale receipt.
