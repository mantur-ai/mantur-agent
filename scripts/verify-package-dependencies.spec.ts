...
describe('package dependency scope', () => {
  it('keeps the measured Host relay roster explicit', () => {
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@deepseek-ai/dsh-mcp-client']).toEqual(['Config', 'inject', 'name'])
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@deepseek-ai/dsh-mcp-client']).toEqual(['apply'])
  })
...
describe('face-aware source classification', () => {
  it('requires an MCP apply consumer to retain the shared peer instance', () => {
    ...
    writeFileSync(join(directory, 'index.ts'), `import { apply, Config, inject, name } from '${mcp}'; export const plugin = { apply, Config, inject, name }`)
