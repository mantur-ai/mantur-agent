...
原生客户端侧边栏底部仅显示黄色馒头图标和余额数字，没有卡片或刷新按钮。页面可见时按 Host 配置的 `balanceRefreshIntervalMs` 间隔同步（漫途配置为 5 秒），登录、账号变化、窗口重新获得焦点和网络恢复时立即读取。刷新期间保留已确认数字并标记无障碍忙碌状态，失败时清除数字。余额不可用时显示“—”，具体状态通过本地化悬浮提示说明。页面隐藏时暂停轮询，返回时立即同步；退出登录和切换账号使旧请求失效。
...
侧栏展开时，余额图标和数字分别与设置图标和文字对齐；收起时保留上下排列的图标和紧凑数字。
