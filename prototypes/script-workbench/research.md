...
核对日期：2026-09-09。结论是先保持「一集一个文本文件」，生产版优先复用仓库已有 Lexical 依赖及基础编辑能力；不复制聊天输入框整个组件，不引入完整剧本应用。本次隔离原型使用原生 textarea，使用 Marked 与 DOMPurify 提供只读 Markdown 渲染，验证选段和文件生命周期；尚未证明 Lexical 完成剧本集成。
