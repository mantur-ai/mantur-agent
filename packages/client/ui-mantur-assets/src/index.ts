...
import type { AssetCommand, AssetEntry, AssetMedia, AssetProposal, AssetSnapshot, AssetState, AssetVersion, PromptEdit, SourcePin } from './types.ts'
...
export class ManturAssets extends TypertRemoteService {
  ...
  @Remote('load') async load(agent: Agent, assetsPath: string, _clipsPath?: string, mediaManifest?: string): Promise<AssetSnapshot> {
    ...
    const state = await this.readState(stateFile, assetsPath); const media = new Map<string, { path: string; sha: string; type: string }>()
  ...
  @Remote('prepare') async prepare(agent: Agent, source: SourcePin, edits: PromptEdit[], instruction: string): Promise<{ requestId: string; source: SourcePin; edits: PromptEdit[] }> { const current = await this.require(agent, source); if (current.sha256 !== source.sha256 || current.version !== source.version) throw new FsError('Source changed; reload before preparing.', 'FS_STALE_VERSION'); const id = randomUUID(); const state = await this.readState((await this.session(agent)).stateFile, source.path); state.proposals.push({ id: id as never, session: String(agent.session.header.cwd), source, draftRevision: state.drafts.at(-1)?.revision ?? 0, instruction, before: edits, edits, status: 'requested' }); await this.writeState(agent, source, state, await this.stateVersion(agent)); return { requestId: id, source, edits } }
  ...
  private async readState(path: string, source: string): Promise<AssetState> { try { return JSON.parse(await this.read(path, this.config.maxBytes)) as AssetState } catch { return emptyState(source) } }
  private async pin(agent: Agent, path: string): Promise<SourcePin> { const target = await this.target(agent, path, true); const stat = await this.ctx.fs.stat(target); if (stat?.type !== 'file') throw new Error('Asset report is not a file'); const text = await this.read(target.displayPath, this.config.maxBytes); return { path: target.displayPath, version: stat.version as AssetVersion, sha256: fingerprint(text) } }
