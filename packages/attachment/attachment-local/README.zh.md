...
图片准入先读取解码器元数据，再拒绝不支持的格式。依赖下限为 sharp 0.35.4，其内置 libheif 1.23.2 修复 [GHSA-rgj7-g3m4-5g8c](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c)。允许的格式仍仅为 PNG、JPEG、WebP 和 GIF；即使解码器能够读取 AVIF 的元数据，该图片也会被拒绝。
