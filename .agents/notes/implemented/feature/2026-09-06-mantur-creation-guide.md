...
The footer declares the optional-Session `conversation.composer.layout.permissions` child slot after the workspace and permits wrapping. Conversation contributes its internal selector, using the original input faces, permission projection, command callback, and shared interaction locks. The layout passes only a boolean display lock, including the composer block. Framework-bound occupancy selects the external or inline position for the declaration lifetime; Mantur owns no permission state or component import. This keeps the official control inline when the alternate layout unloads without adding another ReactNode owner prop or changing model-selection locks.
